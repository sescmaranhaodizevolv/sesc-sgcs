...
function Processos({ currentProfile = 'admin' }: ProcessosProps) {
  ...
  const [reqParaPrioridade, setReqParaPrioridade] = useState<any>(null);
  ...
  const processosConsolidados: ProcessoConsolidado[] = [
    {
      id: 'CONT-2024-001',
      requisitante: 'Carlos Alberto - Alimentação',
      objeto: 'Aquisição de equipamentos de cozinha industrial',
  ...
  const filteredProcessosConsolidados = processosConsolidados.filter(processo => {
    const empresaVencedora = processo.empresaVencedora || '';
    const matchesSearch = empresaVencedora.toLowerCase().includes(searchTermConsolidado.toLowerCase()) ||
  ...
  const filteredDiretas = filteredProcessosConsolidados.filter(
    (processo) => processo.tipo !== 'Licitação (Pesquisa de Preço)'
  );

  const filteredLicitacoes = filteredProcessosConsolidados.filter(
    (processo) => processo.tipo === 'Licitação (Pesquisa de Preço)'
  );
  ...
  // Sorting hooks
  const { items: sortedRequisicoes, requestSort: sortReq, sortConfig: configReq } = useTableSort(filteredRequisicoes);
  const { items: sortedAtribuidas, requestSort: sortAtr, sortConfig: configAtr } = useTableSort(filteredRequisicoesAtribuidas);
  const { items: sortedDiarios, requestSort: sortDiario, sortConfig: configDiario } = useTableSort(filteredProcessosDiarios);
  const { items: sortedDiretas, requestSort: sortDiretas, sortConfig: configDiretas } = useTableSort(filteredDiretas);
  const { items: sortedLicitacoes, requestSort: sortLicitacoes, sortConfig: configLicitacoes } = useTableSort(filteredLicitacoes);
  const { items: sortedTRP, requestSort: sortTRP, sortConfig: configTRP } = useTableSort(filteredTRP);
  const { items: sortedContratos, requestSort: sortContratos, sortConfig: configContratos } = useTableSort(filteredContratos);
  ...
  const paginatedProcessosDiarios = sortedDiarios.slice(startIndex, endIndex);
              ...
              <CardContent className="pt-[0px] pr-[16px] pb-[24px] pl-[16px]">
                <div className="w-full overflow-x-auto">
                  <Table>
                    <TableHeader>
                      <TableRow>
                        <SortableTableHead label="ID Requisição" onClick={() => sortAtr('id')} currentDirection={configAtr?.key === 'id' ? configAtr.direction : null} className="sticky left-0 z-10 min-w-[140px] bg-white" />
                        <SortableTableHead label="Requisitante" onClick={() => sortAtr('requisitante')} currentDirection={configAtr?.key === 'requisitante' ? configAtr.direction : null} className="min-w-[200px]" />
                        <SortableTableHead label="Objeto" onClick={() => sortAtr('objeto')} currentDirection={configAtr?.key === 'objeto' ? configAtr.direction : null} className="min-w-[300px]" />
                        <SortableTableHead label="Responsável" onClick={() => sortAtr('responsavel')} currentDirection={configAtr?.key === 'responsavel' ? configAtr.direction : null} className="min-w-[160px]" />
                        <SortableTableHead label="Prioridade" onClick={() => sortAtr('prioridade')} currentDirection={configAtr?.key === 'prioridade' ? configAtr.direction : null} className="min-w-[140px]" />
                    ...
                    <TableBody>
                      {sortedAtribuidas.map((requisicao) => (
            ...
            <CardContent className="pt-[0px] pr-[16px] pb-[24px] pl-[16px]">
              <div className="w-full overflow-x-auto">
                <Table>
                  <TableHeader>
                    <TableRow>
                      <SortableTableHead label="ID Requisição" onClick={() => sortReq('id')} currentDirection={configReq?.key === 'id' ? configReq.direction : null} className="sticky left-0 z-10 min-w-[140px] bg-white" />
                      <SortableTableHead label="Requisitante" onClick={() => sortReq('requisitante')} currentDirection={configReq?.key === 'requisitante' ? configReq.direction : null} className="min-w-[200px]" />
                      <SortableTableHead label="Objeto" onClick={() => sortReq('objeto')} currentDirection={configReq?.key === 'objeto' ? configReq.direction : null} className="min-w-[300px]" />
                      <SortableTableHead label="Responsável" onClick={() => sortReq('responsavel')} currentDirection={configReq?.key === 'responsavel' ? configReq.direction : null} className="min-w-[160px]" />
                      <SortableTableHead label="Prioridade" onClick={() => sortReq('prioridade')} currentDirection={configReq?.key === 'prioridade' ? configReq.direction : null} className="min-w-[140px]" />
              ...
              <CardContent className="pt-[0px] pr-[16px] pb-[24px] pl-[16px]">
                <div className="w-full overflow-x-auto">
                  <Table>
                    <TableHeader>
                      <TableRow>
                        <SortableTableHead label="ID Processo" onClick={() => sortDiario('id')} currentDirection={configDiario?.key === 'id' ? configDiario.direction : null} className="sticky left-0 z-10 min-w-[140px] bg-white" />
                        <SortableTableHead label="Nº da Requisição" onClick={() => sortDiario('numeroRequisicao')} currentDirection={configDiario?.key === 'numeroRequisicao' ? configDiario.direction : null} className="min-w-[160px]" />
                        <SortableTableHead label="Requisitante" onClick={() => sortDiario('requisitante')} currentDirection={configDiario?.key === 'requisitante' ? configDiario.direction : null} className="min-w-[200px]" />
                        <SortableTableHead label="Objeto" onClick={() => sortDiario('objeto')} currentDirection={configDiario?.key === 'objeto' ? configDiario.direction : null} className="min-w-[300px]" />
                        <SortableTableHead label="Tipo/Modalidade" onClick={() => sortDiario('tipo')} currentDirection={configDiario?.key === 'tipo' ? configDiario.direction : null} className="min-w-[200px]" />
                        <SortableTableHead label="Status" onClick={() => sortDiario('status')} currentDirection={configDiario?.key === 'status' ? configDiario.direction : null} className="min-w-[220px]" />
                        <SortableTableHead label="Responsável" onClick={() => sortDiario('responsavel')} currentDirection={configDiario?.key === 'responsavel' ? configDiario.direction : null} className="min-w-[160px]" />
                        <TableHead className="min-w-[140px]">Data Recebimento</TableHead>
                        <TableHead className="min-w-[140px]">Data Finalização</TableHead>
                        <SortableTableHead label="Empresa Vencedora" onClick={() => sortDiario('empresaVencedora')} currentDirection={configDiario?.key === 'empresaVencedora' ? configDiario.direction : null} className="min-w-[200px]" />
          ...
          <Card className="border border-gray-200">
            ...
            <CardContent className="pt-[0px] pr-[16px] pb-[24px] pl-[16px] space-y-4">
              ...
              <Tabs defaultValue="diretas" className="w-full">
                <TabsList className="grid w-full max-w-4xl grid-cols-4 mb-4 h-9 p-1 bg-muted rounded-md overflow-hidden">
                  <TabsTrigger value="diretas" className="text-center h-7 px-3 py-0 text-sm">
                    Contratações Diretas ({filteredDiretas.length})
                  </TabsTrigger>

                  <TabsTrigger value="licitacoes" className="text-center h-7 px-3 py-0 text-sm">
                    Licitações Homologadas ({filteredLicitacoes.length})
                  </TabsTrigger>

                  {currentProfile === 'admin' && (
                    <TabsTrigger value="trp" className="text-center h-7 px-3 py-0 text-sm">
                      Lista de TRPs ({filteredTRP.length})
                    </TabsTrigger>
                  )}

                  {currentProfile === 'admin' && (
                    <TabsTrigger value="contratos" className="text-center h-7 px-3 py-0 text-sm">
                      Lista de Contratos ({filteredContratos.length})
                    </TabsTrigger>
                  )}
                ...
                <TabsContent value="diretas" className="mt-0 space-y-4">
                  <div className="w-full overflow-x-auto border rounded-lg max-h-[600px]">
                    <Table>
                      <TableHeader>
                        <TableRow>
                          <SortableTableHead label="ID Processo" onClick={() => sortDiretas('id')} currentDirection={configDiretas?.key === 'id' ? configDiretas.direction : null} className="sticky left-0 z-10 min-w-[140px] bg-white" />
                          <SortableTableHead label="Requisitante" onClick={() => sortDiretas('requisitante')} currentDirection={configDiretas?.key === 'requisitante' ? configDiretas.direction : null} className="min-w-[200px]" />
                          <SortableTableHead label="Objeto" onClick={() => sortDiretas('objeto')} currentDirection={configDiretas?.key === 'objeto' ? configDiretas.direction : null} className="min-w-[300px]" />
                          ...
                          <SortableTableHead label="Status" onClick={() => sortDiretas('status')} currentDirection={configDiretas?.key === 'status' ? configDiretas.direction : null} className="min-w-[160px]" />
                ...
                <TabsContent value="licitacoes" className="mt-0 space-y-4">
                  <div className="w-full overflow-x-auto border rounded-lg max-h-[600px]">
                    <Table>
                      <TableHeader>
                        <TableRow>
                          <SortableTableHead label="ID Processo" onClick={() => sortLicitacoes('id')} currentDirection={configLicitacoes?.key === 'id' ? configLicitacoes.direction : null} className="sticky left-0 z-10 min-w-[140px] bg-white" />
                          <SortableTableHead label="Requisitante" onClick={() => sortLicitacoes('requisitante')} currentDirection={configLicitacoes?.key === 'requisitante' ? configLicitacoes.direction : null} className="min-w-[200px]" />
                          <SortableTableHead label="Objeto" onClick={() => sortLicitacoes('objeto')} currentDirection={configLicitacoes?.key === 'objeto' ? configLicitacoes.direction : null} className="min-w-[300px]" />
                          ...
                          <SortableTableHead label="Status" onClick={() => sortLicitacoes('status')} currentDirection={configLicitacoes?.key === 'status' ? configLicitacoes.direction : null} className="min-w-[160px]" />
                          ...
                          <TableHead className="min-w-[140px]">Data Recebimento</TableHead>
                          <TableHead className="min-w-[140px]">Data Finalização</TableHead>
                          <TableHead className="min-w-[140px]">Data Homologação</TableHead>
                          <TableHead className="min-w-[140px]">Tempo Total (Dias)</TableHead>
                      ...
                      <Table>
                        <TableHeader>
                          <TableRow>
                            ...
                            <SortableTableHead label="CNPJ" onClick={() => sortTRP('cnpj')} currentDirection={configTRP?.key === 'cnpj' ? configTRP.direction : null} className="min-w-[180px]" />
                            ...
                            <SortableTableHead label="Vigência" onClick={() => sortTRP('vigencia')} currentDirection={configTRP?.key === 'vigencia' ? configTRP.direction : null} className="min-w-[120px]" />
                            <SortableTableHead label="Aditivos" onClick={() => sortTRP('aditivos')} currentDirection={configTRP?.key === 'aditivos' ? configTRP.direction : null} className="min-w-[100px]" />
                      ...
                      <Table>
                        <TableHeader>
                          <TableRow>
                            ...
                            <SortableTableHead label="CNPJ" onClick={() => sortContratos('cnpj')} currentDirection={configContratos?.key === 'cnpj' ? configContratos.direction : null} className="min-w-[180px]" />
                            ...
                            <SortableTableHead label="Vigência" onClick={() => sortContratos('vigencia')} currentDirection={configContratos?.key === 'vigencia' ? configContratos.direction : null} className="min-w-[120px]" />
                            <SortableTableHead label="Aditivos" onClick={() => sortContratos('aditivos')} currentDirection={configContratos?.key === 'aditivos' ? configContratos.direction : null} className="min-w-[100px]" />
                            <TableHead className="min-w-[100px]">Ações</TableHead>
                          </TableRow>
                        </TableHeader>
                        <TableBody>
      ...
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Defina a nova prioridade para a requisição {reqParaPrioridade?.id}
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
...
export default function AdminProcessosPage() {
  return (
    <div className="flex-1 space-y-4 p-4 md:p-8 pt-6">
      <Processos currentProfile="admin" />
    </div>
  );
}
