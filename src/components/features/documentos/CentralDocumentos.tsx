                  ...
                  <TableHeader>
                    <TableRow>
                      <SortableTableHead
                        ...
                        onClick={() => sortDocumentos('nome')}
                        currentDirection={configDocumentos?.key === 'nome' ? configDocumentos.direction : null}
                        className="sticky left-0 z-10 min-w-[300px] bg-white"
                      />
                      <SortableTableHead
                        label="Empresa"
                        onClick={() => sortDocumentos('empresa')}
                        currentDirection={configDocumentos?.key === 'empresa' ? configDocumentos.direction : null}
                        className="min-w-[160px]"
                      />
                      <SortableTableHead
                        label="Processo"
                        onClick={() => sortDocumentos('processo')}
                        currentDirection={configDocumentos?.key === 'processo' ? configDocumentos.direction : null}
                        className="min-w-[100px]"
                      />
                      <SortableTableHead
                        label="Tipo"
                        onClick={() => sortDocumentos('tipo')}
                        currentDirection={configDocumentos?.key === 'tipo' ? configDocumentos.direction : null}
                        className="min-w-[140px]"
                      />
                      <SortableTableHead
                        label="Tamanho"
                        onClick={() => sortDocumentos('tamanho')}
                        currentDirection={configDocumentos?.key === 'tamanho' ? configDocumentos.direction : null}
                        className="min-w-[100px]"
                      />
                      <SortableTableHead
                        label="Data Upload"
                        onClick={() => sortDocumentos('dataUpload')}
                        currentDirection={configDocumentos?.key === 'dataUpload' ? configDocumentos.direction : null}
                        className="min-w-[120px]"
                      />
                      <SortableTableHead
                        label="Upload Por"
                        onClick={() => sortDocumentos('uploadPor')}
                        currentDirection={configDocumentos?.key === 'uploadPor' ? configDocumentos.direction : null}
                  ...
                  <TableBody>
                    {sortedDocumentos.map((documento) => (
                      <TableRow key={documento.id}>
                        ...
                        <TableCell>
                          <BadgeStatus {...getBadgeMappingForTipoDocumento(documento.tipo)}>
                            {documento.tipo}
                          </BadgeStatus>
                        </TableCell>
