...
import { Plus, Search, Archive, RotateCcw, Pencil, Paperclip, Shield, MoreVertical, Eye, Download, Clock, FileText, History, AlertTriangle, XCircle, Upload, Trash2 } from "lucide-react";
...
export function GerenciadorPenalidades({ viewMode }: GerenciadorPenalidadesProps) {
  ...
  const compradorResponsavelMock = "Maria Silva";
  ...
  const [penalidadesEncerradas, setPenalidadesEncerradas] = useState<typeof penalidades>([
    {
      id: 101,
      empresa: 'Logística BCD S.A',
      processo: '2023-045',
      notificacoes: 1,
      penalidade: 'Atraso na entrega',
      valor: 'R$ 8.500,00',
      status: 'Vencida',
      dataOcorrencia: '10/05/2023',
      dataAplicacao: '15/05/2023',
      responsavel: 'Maria Silva'
    },
    {
      id: 102,
      ...
      processo: '2023-012',
      notificacoes: 3,
      penalidade: 'Descumprimento de especificação',
      valor: 'R$ 32.000,00',
      status: 'Vencida',
      dataOcorrencia: '02/02/2023',
      dataAplicacao: '10/02/2023',
      responsavel: 'João Santos'
    }
  ]);

  const [novosAnexos, setNovosAnexos] = useState<Record<number, { id: string, nome: string, tamanho: string }[]>>({});

  const handleUploadMultiplo = (penalidadeId: number, files: FileList | null) => {
    if (!files) return;
    ...
    setNovosAnexos((prev) => ({
      ...prev,
      [penalidadeId]: [...(prev[penalidadeId] || []), ...newFiles],
    }));
    toast.success(`${newFiles.length} documento(s) anexado(s) com sucesso!`);
  };

  const handleRemoverNovoAnexo = (penalidadeId: number, anexoId: string) => {
    setNovosAnexos((prev) => ({
      ...prev,
      [penalidadeId]: (prev[penalidadeId] || []).filter((a) => a.id !== anexoId),
    }));
  };

  const penalidadesVisiveis = isAdminView
    ? penalidades
    : penalidades.filter((p) => p.responsavel === compradorResponsavelMock);

  const penalidadesEncerradasVisiveis = isAdminView
    ? penalidadesEncerradas
    : penalidadesEncerradas.filter((p) => p.responsavel === compradorResponsavelMock);

  const statusCounts = {
    todas: penalidadesVisiveis.length,
    aplicada: penalidadesVisiveis.filter((p) => p.status === "Aplicada").length,
    contestada: penalidadesVisiveis.filter((p) => p.status === "Contestada").length,
    vencida: penalidadesVisiveis.filter((p) => p.status === "Vencida").length,
    encerradas: penalidadesEncerradasVisiveis.length,
  };
  ...
  const filteredPenalidades = penalidadesVisiveis.filter(penalidade => {
  ...
  const filteredEncerradas = penalidadesEncerradasVisiveis.filter(penalidade => {
    const matchesSearch = penalidade.empresa.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         penalidade.processo.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         penalidade.penalidade.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const totalValor = penalidadesVisiveis
                ...
                <div className="space-y-1.5">
                  ...
                  <Select>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="fornecedor-xyz">Fornecedor XYZ S.A</SelectItem>
                      <SelectItem value="servicos-def">Serviços DEF Eireli</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-1.5">
        ...
        <TabsContent value="ativas">
          <Card className="border border-gray-200">
            <CardHeader className="pt-3 pb-1">
              <CardTitle className="text-xl text-black px-[0px] py-[8px]">Histórico Completo</CardTitle>
            </CardHeader>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      <SortableTableHead
                        label="Empresa"
                        onClick={() => sortPenalidades('empresa')}
                        currentDirection={configPenalidades?.key === 'empresa' ? configPenalidades.direction : null}
                        className="sticky left-0 z-10 min-w-[200px] bg-white"
                      />
                      <SortableTableHead
                        label="Processo"
                        onClick={() => sortPenalidades('processo')}
                        currentDirection={configPenalidades?.key === 'processo' ? configPenalidades.direction : null}
                        className="min-w-[120px]"
                      />
                      <SortableTableHead
                        label="Notificações"
                        onClick={() => sortPenalidades('notificacoes')}
                        currentDirection={configPenalidades?.key === 'notificacoes' ? configPenalidades.direction : null}
                        className="min-w-[120px]"
                      />
                      <SortableTableHead
                        label="Penalidade"
                        onClick={() => sortPenalidades('penalidade')}
                        currentDirection={configPenalidades?.key === 'penalidade' ? configPenalidades.direction : null}
                        className="min-w-[200px]"
                      />
                      <SortableTableHead
                        label="Multa"
                        onClick={() => sortPenalidades('valor')}
                        currentDirection={configPenalidades?.key === 'valor' ? configPenalidades.direction : null}
                        className="min-w-[120px]"
                      />
                      <SortableTableHead
                        label="Status"
                        onClick={() => sortPenalidades('status')}
                        currentDirection={configPenalidades?.key === 'status' ? configPenalidades.direction : null}
                        className="min-w-[140px]"
                      />
                      <SortableTableHead
                        label="Data Ocorrência"
                        onClick={() => sortPenalidades('dataOcorrencia')}
                        currentDirection={configPenalidades?.key === 'dataOcorrencia' ? configPenalidades.direction : null}
                        className="min-w-[140px]"
                      />
                      <SortableTableHead
                        label="Data Aplicação"
                        onClick={() => sortPenalidades('dataAplicacao')}
                        currentDirection={configPenalidades?.key === 'dataAplicacao' ? configPenalidades.direction : null}
                        className="min-w-[140px]"
                      />
                      ...
                      {isAdminView && (
                        <SortableTableHead
                          label="Responsável"
                          onClick={() => sortPenalidades('responsavel')}
                          currentDirection={configPenalidades?.key === 'responsavel' ? configPenalidades.direction : null}
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        <SortableTableHead
                          label="Empresa"
                          onClick={() => sortEncerradas('empresa')}
                          currentDirection={configEncerradas?.key === 'empresa' ? configEncerradas.direction : null}
                          className="sticky left-0 z-10 min-w-[200px] bg-white"
                        />
                        <SortableTableHead
                          label="Processo"
                          onClick={() => sortEncerradas('processo')}
                          currentDirection={configEncerradas?.key === 'processo' ? configEncerradas.direction : null}
                          className="min-w-[120px]"
                        />
                        <SortableTableHead
                          label="Notificações"
                          onClick={() => sortEncerradas('notificacoes')}
                          currentDirection={configEncerradas?.key === 'notificacoes' ? configEncerradas.direction : null}
                          className="min-w-[120px]"
                        />
                        <SortableTableHead
                          label="Penalidade"
                          onClick={() => sortEncerradas('penalidade')}
                          currentDirection={configEncerradas?.key === 'penalidade' ? configEncerradas.direction : null}
                          className="min-w-[200px]"
                        />
                        <SortableTableHead
                          label="Multa"
                          onClick={() => sortEncerradas('valor')}
                          currentDirection={configEncerradas?.key === 'valor' ? configEncerradas.direction : null}
                          className="min-w-[120px]"
                        />
                        <SortableTableHead
                          label="Status"
                          onClick={() => sortEncerradas('status')}
                          currentDirection={configEncerradas?.key === 'status' ? configEncerradas.direction : null}
                          className="min-w-[140px]"
                        />
                        <SortableTableHead
                          label="Data Ocorrência"
                          onClick={() => sortEncerradas('dataOcorrencia')}
                          currentDirection={configEncerradas?.key === 'dataOcorrencia' ? configEncerradas.direction : null}
                          className="min-w-[140px]"
                        />
                        <SortableTableHead
                          label="Data Aplicação"
                          onClick={() => sortEncerradas('dataAplicacao')}
                          currentDirection={configEncerradas?.key === 'dataAplicacao' ? configEncerradas.direction : null}
                          className="min-w-[140px]"
                        />
                        <TableHead className="min-w-[140px]">Tempo de Vigência</TableHead>
                        {isAdminView && (
                          <SortableTableHead
                            label="Responsável"
                            onClick={() => sortEncerradas('responsavel')}
                            currentDirection={configEncerradas?.key === 'responsavel' ? configEncerradas.direction : null}
                            className="min-w-[160px]"
                          />
                        )}
                        <TableHead className="min-w-[100px]">Ações</TableHead>
                                ...
                                <Dialog>
                                  ...
                                  <DialogContent className="max-w-3xl max-h-[80vh] overflow-y-auto">
                                    <DialogHeader>
                                      ...
                                      <DialogDescription>
                                        Processo {penalidade.processo} • {penalidade.empresa}
                                      </DialogDescription>
                                    </DialogHeader>

                                    {/* Área de Upload */}
                                    <div className="mb-6 bg-gray-50 border border-dashed border-gray-300 rounded-lg p-4 text-center">
                                      <label className="cursor-pointer flex flex-col items-center justify-center gap-2">
                                        <div className="p-2 bg-blue-100 rounded-full">
                                          <Upload size={20} className="text-[#003366]" />
                                        </div>
                                        <span className="text-sm font-medium text-[#003366]">Clique para anexar documentos</span>
                                        <span className="text-xs text-gray-500">PDF, DOC, DOCX, JPG (Máx. 10MB)</span>
                                        <input
                                          type="file"
                                          multiple
                                          className="hidden"
                                          accept=".pdf,.doc,.docx,.jpg,.jpeg,.png"
                                          onChange={(e) => handleUploadMultiplo(penalidade.id, e.target.files)}
                                        />
                                      </label>
                                    </div>
                                    ...
                                    <div className="space-y-3 pt-2">
                                      <div className="flex items-center justify-between p-4 border rounded-lg hover:bg-gray-50 transition-colors">
                                        <div className="flex items-center gap-3">
                                          <FileText size={36} className="text-[#003366]" />
                                          <div>
                                            <p className="text-black">Auto_de_Infracao_{penalidade.processo}.pdf</p>
                                            <p className="text-sm text-gray-500">Anexado em {penalidade.dataAplicacao} • 245 KB</p>
                                          </div>
                                        </div>
                                      ...
                                      <div className="flex items-center justify-between p-4 border rounded-lg hover:bg-gray-50 transition-colors">
                                        <div className="flex items-center gap-3">
                                          <FileText size={36} className="text-[#003366]" />
                                          <div>
                                            <p className="text-black">Notificacao_Empresa_{penalidade.processo}.pdf</p>
                                            <p className="text-sm text-gray-500">Anexado em {penalidade.dataAplicacao} • 128 KB</p>
                                          </div>
                                        </div>
                                        <Button size="sm" variant="outline" onClick={() => toast.success('Download iniciado!')}>
                                          <Download size={16} className="mr-2" />
                                      ...
                                      <div className="flex items-center justify-between p-4 border rounded-lg hover:bg-gray-50 transition-colors">
                                        <div className="flex items-center gap-3">
                                          <FileText size={36} className="text-[#155dfc]" />
                                          <div>
                                            <p className="text-black">Defesa_Empresa_{penalidade.processo}.pdf</p>
                                      ...
                                      {novosAnexos[penalidade.id]?.map((anexo) => (
                                        <div key={anexo.id} className="flex items-center justify-between p-4 border border-blue-100 bg-blue-50/30 rounded-lg transition-colors">
                                          <div className="flex items-center gap-3">
                                            <FileText size={36} className="text-[#003366]" />
                                            <div>
                                              <p className="text-black font-medium text-sm">{anexo.nome}</p>
                                              <p className="text-xs text-gray-500">Recém adicionado • {anexo.tamanho}</p>
                                            </div>
                                          </div>
                                          <Button
                                            size="sm"
                                            ...
                                            onClick={() => handleRemoverNovoAnexo(penalidade.id, anexo.id)}
                                          >
